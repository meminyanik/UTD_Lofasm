...
uDistance = 0.6907 ; % 3; % Zenith baseline in meters
outAltMap = mainAltMap; % distdim(3525,'ft','m') + uDistance;
%-------------------------------------------------------------------------%
...
x = innerNED(2,:);
y = innerNED(1,:);
z = repmat(0.6907,1,6); % -innerNED(3,:); z will be passed as a variable
...
X = x;
Y = @(l) z*sind(l) + y*cosd(l); dY = @(l) z*cosd(l) - y*sind(l);
Z = @(l) z*cosd(l) - y*sind(l); dZ = @(l) -z*sind(l) - y*cosd(l);

dCygA = decCygnusA;
rCygA = raCygnusA;
HCygA = (LST + h - rCygA).';
wCygA = @(l) -cosd(dCygA)*sin(HCygA*pi/12)*X + sind(dCygA)*Y(l) + cosd(dCygA)*cos(HCygA*pi/12)*Z(l);
dwCygA = @(l) sind(dCygA)*dY(l) + cosd(dCygA)*cos(HCygA*pi/12)*dZ(l);

dCasA = decCasA;
rCasA = raCasA;
HCasA = (LST + h - rCasA).';
wCasA = @(l) -cosd(dCasA)*sin(HCasA*pi/12)*X + sind(dCasA)*Y(l) + cosd(dCasA)*cos(HCasA*pi/12)*Z(l);
dwCasA = @(l) sind(dCasA)*dY(l) + cosd(dCasA)*cos(HCasA*pi/12)*dZ(l);
...
modelledR = @(l) gainCygA.*reshape(permute(exp(1i*2*pi*f.*(wCygA(l)/c-T)),[1 3 2]),[],6)*ones(6,1) + ...
    gainCasA.*reshape(permute(exp(1i*2*pi*f.*(wCasA(l)/c-T)),[1 3 2]),[],6)*ones(6,1);

modelledRdiff = @(l) gainCygA.*reshape(permute(1i*2*pi/c*f.*dwCygA(l).*exp(1i*2*pi*f.*(wCygA(l)/c-T)),[1 3 2]),[],6)*ones(6,1) + ...
    gainCasA.*reshape(permute(1i*2*pi/c*f.*dwCasA(l).*exp(1i*2*pi*f.*(wCasA(l)/c-T)),[1 3 2]),[],6)*ones(6,1);
...
% New Version
minFunc = @(l) (measuredR - modelledR(l));
JacobianMinFunc = @(l) -modelledRdiff(l);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Test Jacobian
%-------------------------------------------------------------------------%
l = linspace(20,50,10000);
mR = zeros(1,length(l));
for n = 1:length(l)
    x_k_1 = l(n);
